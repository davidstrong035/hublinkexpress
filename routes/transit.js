...
router.post("/transit", async function(req, res){
  try {
    ...
    const updateResult = await userModel.updateOne(
      ...
      { $set: { "progress.transit": status } }
    );

    if (updateResult.modifiedCount === 0) {
      return res.status(400).send({ message: "Couldn't update the transit status" });
    }

    return res.status(200).send({ message: "Package is now on transit" });
  } catch (err) {
    console.error(err);
